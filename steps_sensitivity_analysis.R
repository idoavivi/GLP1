# Steps Filter Sensitivity Analysis
# Test how different steps lower bounds affect activity decline findings
# Testing: 0, 20, 50, 100 steps as lower bounds
# Efficient approach: query data once, apply different filters
...
cat("\n##################################################\n")
cat("STEPS FILTER SENSITIVITY ANALYSIS\n")
cat("Testing lower bounds: 0, 20, 50, 100 steps\n")
cat("##################################################\n\n")
...
# Query Fitbit activity data (with minimal filtering)
...
# Basic cleaning (no steps filter yet - will apply per sensitivity test)
activity_basic <- activity_raw %>%
  mutate(activity_date = as.Date(activity_date)) %>%
  filter(
    steps <= 25000,  # Only upper bound
    !is.na(steps),
...
activity_with_glp1 <- activity_basic %>%
...
# Function to run analysis with specific steps filter
run_analysis_with_steps_filter <- function(activity_data, weight_data, drug_data, steps_lower_bound) {

  cat(sprintf("\n========================================\n"))
  cat(sprintf("Running analysis with steps >= %d\n", steps_lower_bound))
  cat(sprintf("========================================\n\n"))

  # Apply steps filter to activity data
  activity_filtered <- activity_data %>%
    filter(steps >= steps_lower_bound)

  # Calculate 1-90d period activity (with wear time filtering)
  period_1_90d_activity <- activity_filtered %>%
  ...
  for (window_name in names(baseline_windows)) {
    ...
    baseline_activity <- activity_filtered %>%
  ...
  # Merge baseline with 1-90d period
  ...
  merged_data <- baseline_data %>%
    inner_join(period_1_90d_final %>% select(-n_fills), by = "person_id")
...
# Run sensitivity analysis with different steps thresholds
steps_thresholds <- c(0, 20, 50, 100)
sensitivity_results <- list()

for (threshold in steps_thresholds) {
  sensitivity_results[[as.character(threshold)]] <- run_analysis_with_steps_filter(
    activity_with_glp1,
    weight_with_glp1,
    drug_glp1_clean,
    threshold
...
cat("\n\n")
cat("========================================\n")
cat("COMPARISON ACROSS STEPS THRESHOLDS\n")
cat("========================================\n\n")

print(combined_results %>%
        select(steps_lower_bound, n_patients,
               baseline_steps_mean, period_steps_mean, steps_pct_change, steps_p,
               MVPA_change, MVPA_p) %>%
        mutate(across(where(is.numeric), ~round(., 2))))

# Save results
write.csv(combined_results, "steps_sensitivity_results.csv", row.names = FALSE)
cat("\n\nResults saved to: steps_sensitivity_results.csv\n")

# Create visualizations
cat("\nGenerating visualizations...\n")

# Plot 1: Steps change by threshold
p1 <- ggplot(combined_results, aes(x = factor(steps_lower_bound), y = steps_pct_change)) +
  geom_col(fill = "steelblue") +
  geom_text(aes(label = sprintf("%.1f%%\np=%.3f", steps_pct_change, steps_p)),
            vjust = ifelse(combined_results$steps_pct_change < 0, 1.2, -0.5), size = 3) +
  labs(title = "Steps Change by Lower Bound Filter",
...
# Plot 2: MVPA change by threshold
p2 <- ggplot(combined_results, aes(x = factor(steps_lower_bound), y = MVPA_change)) +
  geom_col(fill = "coral") +
  geom_text(aes(label = sprintf("%.1f min\np=%.3f", MVPA_change, MVPA_p)),
            vjust = ifelse(combined_results$MVPA_change < 0, 1.2, -0.5), size = 3) +
  labs(title = "MVPA Change by Lower Bound Filter",
       subtitle = "1-90d period vs baseline",
       x = "Steps Lower Bound",
...
# Plot 3: Sample size by threshold
p3 <- ggplot(combined_results, aes(x = factor(steps_lower_bound), y = n_patients)) +
  geom_col(fill = "darkgreen") +
  geom_text(aes(label = n_patients), vjust = -0.5, size = 4) +
  labs(title = "Sample Size by Lower Bound Filter",
...
# Plot 4: % Sedentary change by threshold
p4 <- ggplot(combined_results, aes(x = factor(steps_lower_bound), y = pct_sedentary_change)) +
  geom_col(fill = "purple") +
  geom_text(aes(label = sprintf("%.1f%%\np=%.3f", pct_sedentary_change, pct_sedentary_p)),
            vjust = ifelse(combined_results$pct_sedentary_change < 0, 1.2, -0.5), size = 3) +
  labs(title = "% Sedentary Time Change by Lower Bound Filter",
...
# Save plots
ggsave("steps_sensitivity_steps_change.png", p1, width = 8, height = 6, dpi = 300)
ggsave("steps_sensitivity_MVPA_change.png", p2, width = 8, height = 6, dpi = 300)
ggsave("steps_sensitivity_sample_size.png", p3, width = 8, height = 6, dpi = 300)
ggsave("steps_sensitivity_pct_sedentary.png", p4, width = 8, height = 6, dpi = 300)

cat("\nPlots saved:\n")
cat("  - steps_sensitivity_steps_change.png\n")
cat("  - steps_sensitivity_MVPA_change.png\n")
cat("  - steps_sensitivity_sample_size.png\n")
cat("  - steps_sensitivity_pct_sedentary.png\n")
