...
table1_characteristics <- c(
  ...
  "  Class II (35-39.9), n (%)",
  "  Class III (≥40), n (%)",
  "GLP-1 prescriptions, median [IQR]",
...
table1_values <- c(
  ...
  sprintf("%.0f [%.0f, %.0f]",
          median(table1_data$n_prescriptions, na.rm = TRUE),
          quantile(table1_data$n_prescriptions, 0.25, na.rm = TRUE),
          quantile(table1_data$n_prescriptions, 0.75, na.rm = TRUE)),
